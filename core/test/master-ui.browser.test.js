...
test("Master Distribution saves recipients, pauses and exposes Node API controls", { skip: !fs.existsSync(CHROMIUM), timeout: 45000 }, async t => {
  ...
  assert.equal(await page.isChecked("#ma-metadata-backups"), true);
  await page.uncheck("#ma-metadata-backups");
  ...
  assert.equal((await node.call("masterApi:status")).config.metadataFallback, false);
