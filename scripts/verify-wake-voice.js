...
async function main() {
  ...
  try {
    ...
    const say = async (text, id, volume = 1) => {
      ...
      const audio = Float32Array.from({ length: (resampled.length - 44) / 2 + 16000 }, (_, i) => 44 + 2 * i < resampled.length ? resampled.readInt16LE(44 + 2 * i) / 32768 : 0);
    ...
    console.log("PASS: real Windows Whisper recognizes two voices, quieter wake speech and a follow-up through KAI's activity detector.");
  } finally {
