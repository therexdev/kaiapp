...
function publish(dir, { env = process.env, gh = (...args) => execFileSync("gh", args, {
  ...
  try {
    ...
    fs.writeFileSync(notes, `Master Koinos AI Node ${version}\n\n[Download the Windows Setup installer](https://github.com/${REPO}/releases/download/${tag}/${setup})\n\nFixes the Windows startup error "Unexpected block-store Compose template" by accepting Windows line endings. Adds a release check that runs the actual packaged Windows node startup with temporary data. Retains the tested block-store missing-record safety patch. Historical queries now report a missing record instead of crashing the service. The private Check historical blocks control verifies the running patch and samples local blocks and receipts; it never starts history or claims a complete index. Existing databases, keys, profiles and full block verification are retained. Keep optional indexes off for the first Stop/Start after installing, then run the historical check and review its result before resuming history. Installing the app does not restart Docker. See [history recovery steps](https://github.com/${REPO}/blob/${env.GITHUB_SHA}/docs/MASTER_NODE.md#history-safety-patch-and-local-checks). Includes the previous bounded metadata backups, restricted RPC gateway and Distribution features.\n\nMaster uses a separate app profile and update channel. Before replacing Free Koinos Node, follow the [migration instructions](https://github.com/${REPO}/blob/${env.GITHUB_SHA}/docs/MASTER_NODE.md#migration-from-free-koinos-node).\n\nThe other Windows EXE is the portable app. The AppImage is for Linux x64. Windows executables are signed and timestamped with the existing Michael Milas publisher identity.\n\nSource: ${env.GITHUB_SHA}\nBuild: https://github.com/${REPO}/actions/runs/${env.GITHUB_RUN_ID}\n\nThe master*.yml files update Master only. The latest*.yml files are compatibility pointers to Alpha ${stable.tag_name}; Test remains on test-build. Versioned binaries and build manifests identify the exact source for each Master build.\n`);
    if (!existing) {
