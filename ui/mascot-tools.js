(function (root, factory) {
  const api = typeof module !== "undefined" && module.exports ? factory(require("./agents"), require("./app-navigation")) : factory(root.KaiAgents, root.KaiAppNavigation);
  if (typeof module !== "undefined" && module.exports) module.exports = api;
  else root.KaiMascotTools = api;
})(typeof window !== "undefined" ? window : globalThis, function (agents, navigation) {
  "use strict";
  ...
  async function run({ question, history = [], chatId = "", contextSize = 4096, signal, json, askModel, confirm, open, status = () => {}, onObservation = () => {} }) {
    const tr = await json("/core/tools", { signal }); abort(signal);
    if (!Array.isArray(tr.tools) || !tr.tools.length) return { context: "App tools are unavailable for this turn. Do not claim to have read current app state or used the web.", trace: [], citations: [] };
    const tools = [...tr.tools, ...(open ? [openTool] : [])], names = tools.map(t => t.name);
    ...
    for (let n = 0; n < 6 && !declined && !simpleRead; n++) {
      ...
      try { if (!await call(action.tool, action.args)) break; }
        ...
        if (tools.find(t => t.name === action.tool)?.sensitive) break;
      }
    }
