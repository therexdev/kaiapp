...
final class VoicePack {
    ...
    final KaiApp app;long downloadId;boolean installing;String status="";
    ...
    void download(){
        ...
        try{archive().delete();DownloadManager.Request r=new DownloadManager.Request(Uri.parse(URL)).setTitle("KAI offline voice input · English").setDestinationUri(Uri.fromFile(archive())).setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED).setAllowedOverRoaming(false).setAllowedOverMetered(!app.prefs.getBoolean("wifi",true));
            ...
            downloadId=app.downloads.enqueue(r);app.prefs.edit().putLong("voice.download",downloadId).apply();status="Downloading English speech pack…";app.changed();
        }catch(Exception e){app.fail("Voice download could not start. Check storage and connection.");}
    }
    void cancel(){if(installing)return;if(downloadId!=-1)app.downloads.remove(downloadId);downloadId=-1;app.prefs.edit().remove("voice.download").apply();archive().delete();status="";app.changed();}
    void poll(){
        ...
        try(Cursor c=app.downloads.query(new DownloadManager.Query().setFilterById(downloadId))){
            ...
            long bytes=c.getLong(c.getColumnIndexOrThrow(DownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR));int state=c.getInt(c.getColumnIndexOrThrow(DownloadManager.COLUMN_STATUS));
            ...
            status=state==DownloadManager.STATUS_PAUSED?"Voice pack waiting for Wi-Fi…":"Downloading voice pack · "+(bytes*100/BYTES)+"%";
