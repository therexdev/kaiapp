...
public class StartupTest {
    ...
    private View control(View view,String label){
        ...
        if(view instanceof ViewGroup)for(int i=0;i<((ViewGroup)view).getChildCount();i++){View result=control(((ViewGroup)view).getChildAt(i),label);if(result!=null)return result;}return null;
    }
    private View control(String label){return control(activity.getWindow().getDecorView(),label);}
    private void chooseVoiceChat(){control("Voice options").performClick();org.robolectric.shadows.ShadowPopupMenu.getLatestPopupMenu().getMenu().performIdentifierAction(2,0);}
    ...
    @Test public void firstLaunchRequiresAccountAndAllFiveTabsOpen() {
        ...
        find("Chat").performClick();assertNull(control("Send message"));assertNotNull(find("Get started · Sign in"));
    }
    @Test public void signedInLocalChatOpensWithoutSendingAnythingOnline() throws Exception {
        signIn();assertNotNull(find("Choose a local model"));assertNotNull(control("Voice options"));assertNull(find("Voice chat"));assertNotNull(control("Web search off"));assertTrue(control("Send message").isEnabled());assertFalse(app.networkAllowed());
    ...
    @Test public void signedInModelSwitchesNeedNoDisconnectDialog() throws Exception {
        ...
        assertNotNull(find("Choose a local model"));assertTrue(control("Send message").isEnabled());
    }
    ...
    @Test public void voiceDownloadProgressDisappearsAfterSetupAndReadyStateStaysInSettings()throws Exception{
        ...
        app.voicePack.downloadId=-1;installedVoice();app.changed();assertNull(find("Voice ready · works offline"));assertEquals(View.GONE,progress.getVisibility());assertEquals("Voice input is ready",org.robolectric.shadows.ShadowToast.getTextOfLatestToast());find("Settings").performClick();assertNotNull(find("English voice input · ready offline"));
    }
    @Test public void inlineVoiceMenuOffersBothModesAndActiveControlFinishesOrStops()throws Exception{
